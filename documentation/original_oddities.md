...
- When Gluminizer ability is combined with a monster with RAISESPEED ability, the combination goes as follows: 1. the cost of movement is computed with the RAISESPEED ability by dividing the cost (in float), 3. the Gluminizer ability is computed by multiplying the float cost, 2. the cost is rounded to the highest int (ceil). Example:
